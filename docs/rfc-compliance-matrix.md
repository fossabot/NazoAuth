...
| FAPI 2.0 Message Signing | FAPI message-signing options for signed authorization requests, signed authorization responses, and signed introspection responses. | Signed request object at PAR and JARM where selected; no fallback to unsigned response after signing failure. Signed introspection is separate and requires RFC 9701 support before any public claim. | Advertise each message-signing option independently. Do not advertise signed introspection until RFC 9701 implementation, metadata gating, resource-server validation, and negative tests exist. |
...
| --- | --- | --- | --- | --- | --- |
| Grant types | Support authorization code, refresh token, and client credentials. Do not support implicit or resource owner password grants. Device Authorization Grant, Token Exchange, JWT bearer grant, CIBA, and DCR remain absent unless explicitly implemented with their own threat model. | OAuth 2.1 draft, RFC 9700, FAPI2 | Required for current profile; deferred grants remain unadvertised. | `src/http/token/dispatch.rs`, `src/http/authorization/request.rs`, token dispatch tests | Keep discovery metadata limited to implemented grants and per-client grant allowlists. |
...
| `private_key_jwt` | Validate signature, `iss`/`sub`/client binding, `aud` exactly equal to AS issuer identifier as a string for FAPI, expiry, `iat`/`nbf` clock skew, `jti`, key status, alg allowlist, and replay. | RFC 7523, RFC 9700, FAPI2, RFC 8725 | Required for JWT client auth. | Client assertion tests | RFC 7523 JWT bearer authorization grant is not implemented and must not be implied. Add FAPI clock-skew tests: accept 0-10s future `iat`/`nbf`, reject >60s. |
...
| Introspection | Support authenticated JSON introspection for resource servers/admin clients. Do not leak token validity to unauthenticated callers. Signed introspection response is a distinct RFC 9701 capability. | RFC 7662, RFC 9700, RFC 9701 | Required for current profile; RFC 9701 deferred until implemented. | Introspection tests | RFC 9701 signed JWT introspection is required before advertising FAPI Message Signing signed-introspection option. Add issuer/audience/key-selection/content-negotiation tests before claiming it. |
...
| RFC 7523 | `private_key_jwt` is required for FAPI; JWT bearer authorization grant is optional and risky without assertion trust policy. | Partial | Keep README scoped to JWT Client Authentication. Add JWT bearer grant only after replay, issuer, subject, and trust policy design. |
...
| RFC 7662 | Required for interoperable resource servers. | Implemented | Keep JSON introspection. Treat signed response as RFC 9701 work. |
...
| RFC 8628 | Valid standard but adds phishing, polling, and UX risks. | Deferred | Implement only with user-code UX, polling limits, expiration, denial, and abuse controls. |
| RFC 8693 | Powerful but high-risk delegation/impersonation surface. | Deferred | Implement only with explicit token-exchange policy model and resource/audience constraints. |
...
| RFC 9701 | Required only if signed introspection is advertised or if the FAPI Message Signing signed-introspection option is claimed. | Deferred | Implement before claiming FAPI signed introspection option; bind `iss`, `aud`, key selection, RS identity, content negotiation, and negative tests. |
...
| OIDC Dynamic Client Registration | Useful but high-risk provisioning surface. | Deferred | Admin client CRUD is not DCR. Implement only with initial access token/software statement policy if adopted. |
...
| OIDC Front-Channel Logout | Browser-mediated and weaker than back-channel. | Deferred | Implement only for interoperability demand; do not advertise now. |
| OIDC Session Management | Browser polling-based legacy-style session feature. | Deferred | Avoid unless required by target clients. |
...
| FAPI 2.0 Message Signing Final | Implement signed authorization requests, signed authorization responses, and signed introspection as independent options layered on FAPI 2.0 Security Profile. | Partial/profile-scoped | Implement RFC 9701 before claiming signed introspection. Do not imply JARM or signed introspection when only signed PAR request objects are supported. |
...
| Advertising JARM, JWE, signed UserInfo, or signed introspection without implementation | Forbidden | Cryptographic metadata overclaiming creates false client assumptions and downgrade/confusion risk. |
...
| P2 | DCR / DCR Management | Useful for ecosystems but expands client trust onboarding. | Initial access token or software statement policy, metadata validation, registration access token lifecycle, audit logs, no automatic high-privilege defaults. |
