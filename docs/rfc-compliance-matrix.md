...
| P2 | DCR hardening and RFC 7592 DCR Management | Baseline RFC 7591 DCR is implemented, but management semantics and richer trust policy expand client lifecycle authority. | Registration access token lifecycle, read/update/delete semantics, audit logs, optional software statement trust, optional remote JWKS fetch policy, and no automatic high-privilege defaults. |
