...
- [x] **NI-007 OpenID Connect CIBA / FAPI CIBA**
  ...
  - OIDF 覆盖：`fapi-ciba-id1-test-plan` 已加入 full matrix；2026-07-02 私有一致性测试环境 targeted rerun 通过，`0 failures`、`0 warnings`、`0 skipped modules`，记录见 `docs/conformance/2026-07-02-ni-006-011-private-oidf-results.md`。
- [x] **NI-008 OpenID Connect Front-Channel Logout**
  ...
  - OIDF 覆盖：`oidcc-frontchannel-rp-initiated-logout-certification-test-plan` 已加入 full matrix；2026-07-02 私有一致性测试环境 isolated run 通过，记录见 `docs/conformance/2026-07-02-ni-006-011-private-oidf-results.md`。
- [x] **NI-009 OpenID Connect Session Management**
  ...
  - OIDF 覆盖：`oidcc-session-management-certification-test-plan` 已加入 full matrix；2026-07-02 私有一致性测试环境 targeted run 通过，记录见 `docs/conformance/2026-07-02-ni-006-011-private-oidf-results.md`。
...
| NI-007 OpenID Connect CIBA / FAPI CIBA | `fapi-ciba-id1-test-plan` 覆盖 FAPI-CIBA AS；另有 RP/client alpha plan。 | 已加入 full matrix 并在 2026-07-02 私有一致性测试环境 targeted rerun 通过；保留本地 backchannel authentication、request object、polling、refresh token 和 sender-constrained token tests。 |
| NI-008 OpenID Connect Front-Channel Logout | `oidcc-frontchannel-rp-initiated-logout-certification-test-plan` 覆盖 OP front-channel logout + RP-initiated logout 组合。 | 已加入 full matrix；2026-07-02 私有一致性测试环境 isolated run 通过，同时保留本地 iframe/redirect escaping tests。 |
| NI-009 OpenID Connect Session Management | `oidcc-session-management-certification-test-plan` 覆盖 OP session management。 | 已加入 full matrix；2026-07-02 私有一致性测试环境 targeted run 通过，同时保留本地 `session_state` 和 iframe status tests。 |
