...
`docs/rfc-compliance-matrix.md` 为准。
...
```powershell
git diff --check
rg -n "FAPI2-CIBA 支持|FAPI 2\.1|OpenID Federation.*已实现|implicit grant.*支持|password grant.*支持" README.md README.zh-CN.md docs --glob '!docs/oauth-best-practice-implementation-plan.zh-CN.md'
rg -n "^- \[x\].*(部分完成|profile-scoped|外部边界|未实现)|^\s+- 状态：完成 /" docs/oauth-best-practice-implementation-plan.zh-CN.md
```
...
- `docs/configuration.md` 已记录生产 TLS/HSTS、reverse proxy、trusted mTLS header、CORS 和 cookie policy 边界。
...
- [ ] **M2-03：保持 FAPI2 precision regression**
