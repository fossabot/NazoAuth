...
本文说明仓库维护的 OpenID Foundation Conformance Suite 完整矩阵。矩阵包含 23 个 plan；针对 TP/PS 的新增检查应映射到这些 plan 的覆盖范围，而不是另开一个临时矩阵。
...
| 21 | OIDC Dynamic OP | 运行完整的 code-flow dynamic certification plan，覆盖远程 `jwks_uri`、客户端密钥轮换、精确注册的外部 `request_uri`、签名 Request Object、签名 UserInfo、展示元数据（`logo_uri`、`policy_uri`、`tos_uri`）和动态响应加密元数据。 |
| 22 | OIDC Form Post OP | 通过浏览器流程验证成功与错误授权响应的 `response_mode=form_post`。 |
| 23 | OIDC Third-Party Initiated Login OP | 验证 `initiate_login_uri` 动态注册回读，以及非 HTTPS 元数据拒绝。 |
...
- `private_key_jwt / DPoP / OpenID Connect / authorization code` 是 TP/PS 改动面的主要单 plan；完整回归以 23-plan 矩阵为准。
...
因此，临时 targeted plan-set 只适合开发期间快速定位问题；正式回归和证据记录应引用 23-plan 完整矩阵。

## Expected Skip 策略

当前官方 workflow 在 basic-dynamic 与 full-dynamic 配置中各允许相同的 2 个
逻辑 expected skips（共 4 条记录）：
...
和 unsigned Request Object。完整 Dynamic OP plan 会实际验证签名且精确注册的外部
`request_uri`。包含这些 expected skips 的 workflow run
