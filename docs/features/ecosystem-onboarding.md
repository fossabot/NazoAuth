...
| CIBA client | Register `poll` or `ping` metadata only when CIBA is enabled. Ping requires an exact HTTPS notification endpoint and never carries tokens. | Confidential `private_key_jwt` or mTLS client authentication, signed backchannel requests, mTLS sender-constrained tokens for the FAPI-CIBA ID1 profile, and TLS 1.3-only outbound Ping delivery. | CIBA endpoints and grant metadata are absent unless `ENABLE_CIBA=true`; Push and `user_code` are rejected. The compatibility profile remains separate from internal `fapi2-ciba` hardening. | Invalid signatures/audiences/algorithms, polling violations, unsafe notification targets, TLS 1.2-only notification endpoints, redirects, expired `auth_req_id` values, and unbounded retries fail closed. |
